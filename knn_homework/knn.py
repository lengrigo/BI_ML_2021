...
class KNNClassifier:
    ...
    def compute_distances_one_loop(self, X):
        ...
        distances = np.zeros((X.shape[0], self.train_X.shape[0]))
        for i in range(X.shape[0]):
            difference = np.abs(X[i] - self.train_X)
            distances[i] = np.sum(difference, axis=-1)
        return distances
    ...
    def compute_distances_no_loops(self, X):
        ...
        difference = np.abs(X[:, np.newaxis] - self.train_X)
    ...
    def predict_labels_binary(self, distances):
        ...
        for i in range(n_test):
            neighbor_indexes = distances[i].argsort()[:k]
            neighbor_classes = self.train_y[neighbor_indexes]
